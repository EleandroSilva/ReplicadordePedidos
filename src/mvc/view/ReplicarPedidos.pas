...
uses
  ...
  Uteis.Interfaces, ppDB, ppDBPipe, ppComm, ppRelatv, ppProd, ppClass, ppReport,
  ppBands, ppRichTx, ppStrtch, ppSubRpt, ppCtrls, ppPrnabl, ppCache,
  ...
  begin
    ...
    try
      try
        ...
        if MessageBox(handle, 'Deseja imprimir replicação do pedido?', 'Aviso', mb_IconQuestion or mb_Yesno) = mrYes then
        begin
          FController
            .FactoryDAO
              .DAOPedidos
                .GetbyId(FIdPedidoAtual)
                .DataSet(dsPedidos);
...
procedure TfrmReplicarPedidos.btnImprimirClick(Sender: TObject);
begin
  Imprimir;
end;
...
procedure TfrmReplicarPedidos.Imprimir;
begin
  if not dsPedidos.DataSet.IsEmpty then
  begin
    FController
      .FactoryDAO
        .DAOPessoa
          .GetbyId(dsPedidos.DataSet.FieldByName('IdPessoa').AsString)
          .DataSet(dsPessoa);
    ...
    FController
      .FactoryDAO
        .DAOPrazoPagamento
          .GetbyId(dsPedidos.DataSet.FieldByName('IdPagamento').AsString)
            .DataSet(dsPrazoPagamento);

    pprPedido.Print;
  end
  else
    ShowMessage('Para imprimir pedido, antes encontrar o mesmo!');
end;
...
begin
  ...
  try
    ...
    cdsPedidoPagamentos.Active := True;
    LItem := 0;
    LDataSource.DataSet.First;
    while not LDataSource.DataSet.Eof do
    begin
      ...
      cdsPedidoPagamentos.FieldByName('NumeroPagamento')    .AsInteger   := LItem;//LDataSource.DataSet.FieldByName('NumeroPagamento') .AsInteger;
...
procedure TfrmReplicarPedidos.edtPesquisaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if not cbFiltroComVirgula.Checked then
